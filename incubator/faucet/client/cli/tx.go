...
func GetTxCmd(storeKey string, cdc *codec.AminoCodec) *cobra.Command {
...
func GetCmdMint(cdc *codec.AminoCodec) *cobra.Command {
...
// GetCmdWithdraw is the CLI command for mining coin
func GetCmdMintFor(cdc *codec.AminoCodec) *cobra.Command {
	return &cobra.Command{
...
func GetPublishKey(cdc *codec.AminoCodec) *cobra.Command {
	return &cobra.Command{
		Use:   "publish",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// check local key
			armor, err := kb.Key(cliCtx.GetFromName())
			if err != nil {
				return err
			}

			msg := types.NewMsgFaucetKey(cliCtx.GetFromAddress(), armor.GetName())
			err = msg.ValidateBasic()
			if err != nil {
				return err
			}
...
func GetCmdInitial(cdc *codec.AminoCodec) *cobra.Command {
	return &cobra.Command{
		Use:   "init",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cdc.LegacyAmino.MustUnmarshalJSON(res, &rkey)
